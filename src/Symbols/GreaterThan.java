...
public class GreaterThan extends Square {

    public static final char icon = '>';

    public GreaterThan(Factage program, int y, int x) {
        super(program, y, x);
    ...
    public void updateSquare(boolean[][] updated) {
        ...
        if(programArray[y][x - 1].isOperable() && programArray[y][x + 1].isOperable()) {
            boolean greaterThan = programArray[y][x - 1].getDynType().getOperableValue() > programArray[y][x + 1].getDynType().getOperableValue();
            DynType<Integer> dt = new DynType<>(greaterThan ? 1:0);
